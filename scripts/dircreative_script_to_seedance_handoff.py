    ...
    *,
    ...
    for binding in document.get("bindings", []):
        ...
        if isinstance(reference_id, str):
            stress_reference = next(
                (
                    reference
                    for reference in (stress_asset or {}).get("references", [])
                    if isinstance(reference, dict)
                    and reference.get("reference_id") == reference_id
                ),
                None,
            )
            contract = (
                stress_asset.get("character_sheet_contract", {})
                if isinstance(stress_asset, dict)
                else {}
            )
            if (
                stress_reference is None
                or stress_reference.get("role") != "supporting"
                or stress_reference.get("source_kind") != binding.get("source_kind")
                or stress_reference.get("relative_path") != relative
                or stress_reference.get("sha256") != binding.get("sha256")
                or stress_reference.get("rights_status") != "verified"
                or stress_asset.get("asset_version") != binding.get("asset_version")
                or (
                    binding.get("source_kind") == "character_detail_sheet"
                    and contract.get("detail_reference_id") != reference_id
                )
            ):
                add_error(
                    errors,
                    "binding_reference_provenance_mismatch",
                    f"{asset_id}:{reference_id}",
                )
        else:
            active_reference = None
            if isinstance(stress_asset, dict):
                contract = stress_asset.get("character_sheet_contract")
                if isinstance(contract, dict):
                    active_reference = next(
                        (
                            reference
                            for reference in stress_asset.get("references", [])
                            if isinstance(reference, dict)
                            and reference.get("reference_id")
                            == contract.get("active_reference_id")
                        ),
                        None,
                    )
            if (
            ...
            if (
                ...
                and active_reference.get("source_kind") != binding.get("source_kind")
            ):
                add_error(errors, "binding_active_source_kind_mismatch", asset_id)
    if (
...
def semantic_errors(document: dict[str, Any]) -> list[str]:
    ...
    canonical_asset_ids: set[str] = set()
    supporting_binding_keys: set[tuple[str, str]] = set()
    for binding in bindings:
        ...
        if isinstance(asset_id, str):
            reference_id = binding.get("reference_id")
            if isinstance(reference_id, str):
                supporting_key = (asset_id, reference_id)
                if supporting_key in supporting_binding_keys:
                    add_error(
                        errors,
                        "binding_reference_id_duplicate",
                        f"{asset_id}:{reference_id}",
                    )
                supporting_binding_keys.add(supporting_key)
            else:
                if asset_id in canonical_asset_ids:
                    add_error(errors, "binding_asset_id_duplicate", asset_id)
                canonical_asset_ids.add(asset_id)
        ...
        if source_kind == "exact_graphic_reference" and (
            attached
            or status != "planning_only"
            or binding.get("user_lock_status") != "planning_only"
            or binding.get("direct_input_policy") != "planning_only"
        ):
            add_error(errors, "exact_graphic_direct_binding_forbidden", binding_id)
        if (
            source_kind == "character_detail_sheet"
            and attached
            and not isinstance(binding.get("reference_id"), str)
        ):
            add_error(errors, "supporting_reference_id_required", binding_id)
        if (
            source_kind
            in {"character_master_sheet", "character_state_sheet", "character_headless_sheet"}
            and attached
            and isinstance(binding.get("reference_id"), str)
        ):
            add_error(errors, "canonical_character_binding_reference_forbidden", binding_id)
...
def self_test() -> tuple[list[str], dict[str, Any]]:
    from dircreative_script_to_seedance_selftest import run_self_test
