...
class CanonicalRoleCompilationTests(unittest.TestCase):
    ...
    def test_five_foundation_roles_get_canonical_constraints_without_changing_create_or_ratio(self):
        ...
        self.assertTrue(any('anatomical left and right side views' in item and 'both sides are required' in item for item in normal))
        self.assertTrue(any('near-profile three-quarter angle is explicitly requested' in item for item in normal))
