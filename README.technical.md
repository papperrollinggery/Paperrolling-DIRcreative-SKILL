# DIRcreative — technical guide

[English overview](README.md) · [中文介绍](README.zh-CN.md)

This guide preserves detailed workflow, integration, validation and verified-release installation instructions. Product and dependency-installation information is maintained in the language-specific overviews. Historical verification results below apply only to their stated versions.
...
![DIRcreative introduction](docs/assets/marketing-20260908/intro-en.png)

*AI-generated illustrative artwork.*

DIRcreative 不是“输入一句话、吐出一堆提示词”的黑盒。它先判断任务是局部修改、完整开发还是交付审计，再只加载对应合同。局部任务直接交付修改结果；只有真实方向冲突、生成授权或客户交付才停下来询问。

当前源码版本为 `v0.9.6`；已发布版本与下载以 [GitHub Releases](https://github.com/papperrollinggery/Paperrolling-DIRcreative-SKILL/releases) 为准。本版用公共入口完成资产准备与一次批次审阅，复用未变化的候选质量证据，减少生产任务手写协议和重复看图；保留五视图、真实依赖与最终采纳边界。成人体型和服装剪裁按明确设计传递，不以固定 A-pose 或默认保守服装覆盖；视频提示词在镜头正文前统一采用十二项电影质感前缀。真实生成和输出审核仍有服务与模型限制，不保证每次出图或普遍一次成功。
...
以下命令在 `v0.9.6` tag 与 Release 实际发布后生效。

```bash
gh release download v0.9.6 \
  --repo papperrollinggery/Paperrolling-DIRcreative-SKILL \
  --pattern 'dircreative-0.9.6.tar.gz' \
  --pattern 'SHA256SUMS'
```
...
ARTIFACT="$(pwd)/dircreative-0.9.6.tar.gz"
