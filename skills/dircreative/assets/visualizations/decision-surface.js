(() => {
  ...
  if (spatial) {
    ...
    const statesFor = (view) => view.entity_states || spatial.entity_states_by_phase?.[view.phase] || spec.presentation.spatial_scene?.entities?.map((entity) => ({...entity, position: view.entity_positions?.[entity.id]})) || (spatial.colors || []).map((entity) => ({...entity, position: view.entity_positions?.[entity.id]}));
    ...
    function drawFloorplan(svg, view) {
      ...
      const sourcePaths = (spec.presentation.spatial_scene?.paths || []).map((path) => ({points: path.points, color: (statesFor(view).find((entity) => entity.id === path.entity_id) || {}).color, dashed: true}));
      const cameraSpec = (spec.presentation.spatial_scene?.cameras || []).find((candidate) => candidate.shot_id === view.shot_id);
      const sourceCamera = cameraSpec ? {points: [[cameraSpec.position[0]-.02, cameraSpec.position[1]-.02], [cameraSpec.position[0]+.02, cameraSpec.position[1]-.02], cameraSpec.look_at], color: 'camera', closed: true, dashed: true} : null;
      const shapes = (view.top || []).some((shape) => shape.label) ? view.top || [] : [...sourceFeatures, ...sourcePaths, ...(sourceCamera ? [sourceCamera] : [])];
    ...
    function candidateView() { const phase = phaseSelect?.value || 'initial'; const view = cameraSelect ? spatial.views.find((item) => item.shot_id === cameraSelect.value && item.phase === phase) : spatial.overview?.find?.((item) => item.phase === phase) || spatial.overview || spatial.views?.find((item) => item.phase === phase) || spatial.views?.[0]; return view?.frame_layers && spatial.frame_primitives ? {...view, frame: view.frame_layers.flatMap((token) => spatial.frame_primitives[token] || [])} : view; }
