...
若我没有明确推进方式，先用原生询问框选择讨论共创、直接执行或前期关键节点讨论；复用完整brief与已有授权，不重复问已确定的事实。
按请求区分客户故事、技术前期和真实媒体。导演组与创意组的联合/并行请求使用真实可用子代理，不能模拟已派发。
保持用户给出的故事时长、模型、人物、台词与来源；生成单元按所选模型当前能力和表演容量划分，不套固定15秒上限。
讨论按需要自然进行，方向成熟后展示具体制作范围，用原生框选择开始或继续调整。确认开始后连续完成产物、自检和返工，不逐阶段询问。未授权图片/视频时只交付对应文本或提示词，不重复索要已有授权。
